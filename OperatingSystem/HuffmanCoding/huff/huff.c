...
void print_bin(uint64_t n, int s) {
   for (int i = 0; i < s; ++i)
      printf("%d", n & (1UL << (63 - i)) ? 1 : 0);

   printf("\n");
}

void traverse(struct hnode *n) {
   if (n->sym != 255) {
      printf("%d ", n->sym);
      print_bin(n->code[0], n->len);
   }

   if (n->lft) traverse(n->lft);
   if (n->rgt) traverse(n->rgt);
}
